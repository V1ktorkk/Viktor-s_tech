...
public void addCat(){
    ...
    public void addCatRelationship(){
        KotikiEntity cat1 = new KotikiEntity("Черных", "бродяга", Colors.Rainbow, new Date(Calendar.getInstance().getTime().getTime()), 1);
        KotikiEntity cat2 = new KotikiEntity("Белун", "не бродяга", Colors.Grey, new Date(Calendar.getInstance().getTime().getTime()), 1);
        kotikiService.save(cat1);
        kotikiService.save(cat2);

        when(kotikiDao.getById(1)).thenReturn(cat1);
        when(kotikiDao.getById(2)).thenReturn(cat2);
